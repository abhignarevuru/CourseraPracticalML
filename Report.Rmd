...
Which fields have high correlations with the classe?
...
Even the best correlations with classe are hardly above 0.3, let's check visually if it's indeed hard to use these 2 as possible simple linear predictors.
...
Let's identify variables with high correlations amongst each other in our set, so we can possibly exclude them from the pca or training. We will check afterwards if these modifications to the dataset make the model more accurate, perhaps faster.
...
We'll use 200 trees, because I've already seen that the error rate doesn't decline a lot after say 50 trees, but we still want to be thorough. Also we will time each of the 4 random forest models to see if when all else is equal one pops out as the faster one.
